...
class XLSX(object):
    ...
    def ind_to_rowcol(self,row,col):
        """
        takes python index locations of row and column and returns the cell location used by excel
        snagged from http://stackoverflow.com/questions/23861680/convert-spreadsheet-number-to-column-letter
        """
        div=col
        string = ""
        while div>0:
...
at least one of name, formula, start, or end must be specified for each species
    ...
    def rowcol_to_ind(self,rowcol):
        """
        takes an excel row and column location and converts it to list indices
        snagged from http://stackoverflow.com/questions/7261936/convert-an-excel-or-spreadsheet-column-letter-to-its-number-in-pythonic-fashion
        """
        if 'string' not in self.__dict__:
            self.string = __import__('string')
        alpha = ''
        numeric = ''
        for x in rowcol: # split into alpha and numeric segments
            if x.isalpha():
                alpha += x
            elif x.isdigit():
                numeric += x
